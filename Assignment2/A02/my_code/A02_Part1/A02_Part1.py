...
def my_main(spark, my_dataset_dir):
    ...
    # --------------------------------------------------------
     ...
     # Create views for departures and arrivals
    inputDF.createOrReplaceTempView("trips")

    # Write SQL query to compute departures and arrivals for each station
    solutionDF = spark.sql("""
        SELECT 
            station,
            SUM(is_departure) AS num_departure_trips,
            SUM(is_arrival) AS num_arrival_trips
        FROM (
            SELECT 

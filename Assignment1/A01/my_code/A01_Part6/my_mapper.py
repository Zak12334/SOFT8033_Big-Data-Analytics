...
def my_map(my_input_stream, my_output_stream, my_mapper_input_parameters):
    # Initial Thoughts:
    # - The goal is to track movements of a specific bike, particularly when it's transported by trucks.
    # - Trucks move bikes between stations when there's no user trip record between those stations.
    # - Mapper's job: Extract all trips for the target bike_id, recording start/end times and stations.
    # - We'll use the universal key to ensure all trips for this bike go to the same reducer.

     # Get bike_id to track
    bike_id = int(my_mapper_input_parameters[0])
    ...
    for line in my_input_stream:
        ...
        # If this trip involves our bike
        if trip and len(trip) == 16 and trip[11] == bike_id:
            # Output format: start_time, stop_time, start_station_name, stop_station_name
            # Using station names as required by the reference solution
            my_output_stream.write(f"universal\t({trip[0]}, {trip[1]}, {trip[4]}, {trip[8]})\n")
